...
# **issue_list_issues**
> list[Issue] issue_list_issues(owner, repo, state=state, labels=labels, q=q, type=type, page=page, limit=limit)
...
try:
    # List a repository's issues
    api_response = api_instance.issue_list_issues(owner, repo, state=state, labels=labels, q=q, type=type, page=page, limit=limit)
    pprint(api_response)
except ApiException as e:
